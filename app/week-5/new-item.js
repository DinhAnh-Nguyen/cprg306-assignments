...
export default function NewItem() {
    ...
    return (
        <form
            ...
            className="flex flex-col items-center bg-slate-900 p-6 rounded-lg shadow-lg space-y-4 w-full max-w-sm">
            <input
                type="text"
                value={name}
            ...
            <div className="flex w-full space-x-2">
                <div className="flex items-center space-x-2 bg-white p-2 rounded-lg flex-grow">
                    ...
                    <button type="button" className={`${buttonBaseStyle} ${decrementButtonStyle}`} onClick={decrement}>-</button>
                    <button type="button" className={`${buttonBaseStyle} ${incrementButtonStyle}`} onClick={increment}>+</button>
                </div>
                <select
                    value={category}
            ...
            <div className="w-full max-w-sm">
                <button type="submit" className={`w-full ${buttonBaseStyle} ${submitButtonStyle}`}>Submit</button>
            </div>
        </form>
    );
}